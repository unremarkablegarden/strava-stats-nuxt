...
.container.min-h-screen.py-8.mx-auto
	...
	.flex.justify-between.items-center.mb-8
		h1.text-3xl.font-bold.text-gray-900 Lifting
	...
	LiftingStats(:activities="sortedActivities").mb-8
...
const liftingActivities = computed(() => {
	if (!activities.value) return []
	return activities.value.filter(activity => activity.type === 'WeightTraining')
})

const filteredActivities = computed(() => {
	if (timeFilter.value === 'all') return liftingActivities.value
	const currentYear = new Date().getFullYear()
	return liftingActivities.value.filter(activity => {
